...
import time
import pickle
#import parse_svg_for_path_following_v01
from parse_svg_for_path_following_v01 import *

# TUNABLE CONTROL PARAMETERS
Vmax = 0.6  # Maximum allowed speed
Beta = 7  # gain for Lateral correction speed
gamma = 4  # gain for Angular correction speed
...
#   import stage settings from svg file
    ...
    # CONVERT from the "stage_settings" above: # path_specs = convert_stage_settings_to_path_specs(starting_xytheta_of_path, stage_settings, wheel_width_of_robot)
   # path_specs = ME439_Robot.convert_stage_settings_to_path_specs(np.array([0,0,0]),stage_settings,robot0.wheel_width)

    #imported svg file
    path_specs = convert_svg_to_path_specs(svg_file)
...
##        print(active_controller_left.current, active_controller_left.target, active_controller_right.current, active_controller_right.target)
        ...
        robot0.compute_path_tracking_variables(path_specs[segment,:])
        # check if old segment is done. If so, switch to next segment.
        if robot0.estimated_segment_completion_fraction > 1.0:
            segment += 1
            if segment >= path_specs.shape[0] :
                break
        ...
        robot0.set_wheel_speeds_for_path_tracking(Vmax, Beta, gamma)
        active_controller_left.set_target(robot0.left_wheel_speed)
        active_controller_right.set_target(robot0.right_wheel_speed)
